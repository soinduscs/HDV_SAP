...
{
    public partial class frmOrdenFabricacionINS7 : Form
    {
        public frmOrdenFabricacionINS7()
        {
            InitializeComponent();
        }

        private void frmOrdenFabricacionINS7_Load(object sender, EventArgs e)
        {
        ...
        private void carga_grilla1()
        {
            ...
            try
            {
                ...
                string cfecha1, cfecha2, cMuestra;
                int nDocEntryRef;
                ...
                objproducto.cls_Consultar_Lista_DeclaracionInsumosv3(cfecha1, cfecha2);
                ...
                for (int i = 0; i <= dTable.Rows.Count - 1; i++)
                {
                    ...
                    grilla[0] = nDocEntryRef.ToString();
                    grilla[8] = nDocEntry.ToString();
                    ...
                    if (chk_solo_propias.Checked == true)
                    {
                        cMuestra = "N";

                        if (nUserSign == VariablesGlobales.glb_User_id)
                        {
                            cMuestra = "S";
        ...
        private void btn_abrir_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                nDocEntry = Convert.ToInt32(Grid1[8, fil].Value.ToString());
            }
            catch
            {
                nDocEntry = -1;
            ...
            frmOrdenFabricacionINS6 f2 = new frmOrdenFabricacionINS6();
        ...
        private void btn_exportar_excel_Click(object sender, EventArgs e)
        {
            ...
            int nDocEntry, nDocEntry_SolicitudTransferencia;
            ...
            {
                nDocEntry = 0;
                nDocEntry_SolicitudTransferencia = 0;

                try
                {
                    nDocEntry = Convert.ToInt32(Grid1[8, j].Value.ToString());
                }
                catch
                {
                    nDocEntry = -1;
                ...
                try
                {
                    nDocEntry_SolicitudTransferencia = Convert.ToInt32(Grid1[0, j].Value.ToString());
                ...
                int Number_ojdt, nDocEntry_TransferenciaStock;

                DateTime dFecha_Salida, dFecha_Documento;

                string[] grilla;
                grilla = new string[40];
                ...
                for (int i = 0; i <= dTable.Rows.Count - 1; i++)
                {
                    ...
                    try
                    {
                        nDocEntrySalida = int.Parse(dTable.Rows[i]["DocEntry_Salida"].ToString());
                    }
                    catch
                    {
                        nDocEntrySalida = 0;
                    ...
                    try
                    {
                        Number_ojdt = int.Parse(dTable.Rows[i]["TransId_RegistroDiario"].ToString());
                    }
                    catch
                    {
                        Number_ojdt = 0;
                    ...
                    try
                    {
                        dFecha_Salida = Convert.ToDateTime(dTable.Rows[i]["DocDate_Transferencia"].ToString());
                    }
                    catch
                    {
                    ...
                    try
                    {
                        nDocEntry_TransferenciaStock = int.Parse(dTable.Rows[i]["DocEntry_Transferencia"].ToString());
                    }
                    catch
                    {
                        nDocEntry_TransferenciaStock = 0;
                    }
                    ...
                    if ((cItemCode_D != "") && (nCantidad_D > 0))
                    {
                        ...
                        if (nDocEntry_SolicitudTransferencia > 0)
                        {
                            grilla[29] = nDocEntry_SolicitudTransferencia.ToString();

                        }

                        grilla[30] = "";

                        if (nDocEntry_TransferenciaStock > 0)
                        {
                            grilla[30] = nDocEntry_TransferenciaStock.ToString();
            ...
            try
            {
                ...
                oSheet.Cells[1, 1] = "Solicitud Transferencia";
                oSheet.Cells[1, 2] = "Fecha";
                oSheet.Cells[1, 3] = "Área";
                oSheet.Cells[1, 4] = "Turno";
                oSheet.Cells[1, 5] = "Responsable";
                oSheet.Cells[1, 6] = "Estado";
                oSheet.Cells[1, 7] = "Código";
                oSheet.Cells[1, 8] = "Descripción";
                oSheet.Cells[1, 9] = "U.Med";
                oSheet.Cells[1, 10] = "Lote";
                ...
                oSheet.Cells[1, 17] = "Transferencia Stock";
                oSheet.Cells[1, 18] = "Fecha Proceso";
                oSheet.Cells[1, 19] = "Salida Mercancía";
                oSheet.Cells[1, 20] = "Registro en el diario";

                //Format A1:D1 as bold, vertical alignment = center.
                oSheet.get_Range("a1", "t1").Font.Bold = true;
                oSheet.get_Range("a1", "t1").VerticalAlignment = Excel.XlVAlign.xlVAlignCenter;

                for (int i = 0; i <= Grid2.RowCount - 1; i++)
                {

                    oSheet.Cells[2 + i, 1] = Grid2[29, i].Value.ToString();
                    oSheet.Cells[2 + i, 2] = Grid2[24, i].Value.ToString();
                    oSheet.Cells[2 + i, 3] = Grid2[25, i].Value.ToString();
                    oSheet.Cells[2 + i, 4] = Grid2[26, i].Value.ToString();
                    oSheet.Cells[2 + i, 5] = Grid2[27, i].Value.ToString();
                    oSheet.Cells[2 + i, 6] = Grid2[28, i].Value.ToString();
                    oSheet.Cells[2 + i, 7] = Grid2[1, i].Value.ToString();

                    oSheet.Cells[2 + i, 8] = Grid2[2, i].Value.ToString();
                    oSheet.Cells[2 + i, 9] = Grid2[3, i].Value.ToString();
                    oSheet.Cells[2 + i, 10] = Grid2[4, i].Value.ToString();
                    ...
                    oSheet.Cells[2 + i, 17] = Grid2[30, i].Value.ToString();
                    oSheet.Cells[2 + i, 18] = Grid2[19, i].Value.ToString();
                    oSheet.Cells[2 + i, 19] = Grid2[18, i].Value.ToString();
                    oSheet.Cells[2 + i, 20] = Grid2[20, i].Value.ToString();
